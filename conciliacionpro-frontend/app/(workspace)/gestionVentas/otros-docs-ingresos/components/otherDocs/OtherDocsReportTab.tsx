...
const ALL_COLUMNS: ColDef[] = [
  ...
  { key: "non_fiscal_doc_code",              label: "Cód. documento",     defaultOn: true,  group: "Identificación" },
  ...
  { key: "grand_total",                      label: "Monto",              defaultOn: true,  group: "Montos" },
...
export default function OtherDocsReportTab({ companyId }: Props) {
  ...
  async function handleExport(fmt: "xlsx" | "csv") {
    ...
    try {
      let q = supabase
        .from("trade_docs")
        .select(
          "doc_type,non_fiscal_doc_code,status,issue_date,series,number,reference," +
          "counterparty_identifier_snapshot,counterparty_name_snapshot," +
          "currency_code,grand_total,balance,created_at"
        )
        .eq("company_id", companyId)
        .eq("doc_class", "NON_FISCAL")
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Otros Docs. Ingresos");

      const date = new Date().toISOString().slice(0, 10);
      const filename = `otros_docs_ingresos_${date}.${fmt === "csv" ? "csv" : "xlsx"}`;
