...
import type { OtherDocListFilters, NumericFilterValue } from "./types";
    ...
    <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3">
      ...
      <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
        <select
          ...
          onChange={(e) =>
            onChange({
              op: e.target.value as NumericFilterValue["op"],
              value1: value.value1,
        ...
        <input
          ...
          placeholder={value.op === "between" ? "Desde" : "Valor"}
        />
        {value.op === "between" ? (
          <input
            type="number"
...
export default function OtherDocsFiltersModal({
  open, activeTab, filters, setFilters, onClose, onClear, resultCount,
}: Props) {
  const inputCls =
    "h-10 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm outline-none focus:border-[#123b63]";
  ...
  return (
    <BaseModal
      ...
      subtitle={`Otros Ingresos • ${activeTab === "drafts" ? "Borradores" : "Registrados"}`}
      onClose={onClose}
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setFilters((f) => ({ ...f, counterparty_identifier: e.target.value }))}
