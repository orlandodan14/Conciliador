-- =========================================================
-- 20260511000001_fix_payments_counterparty_snapshots.sql
--
-- Problema: los cobros creados por process_other_doc_import_batch
-- no guardaban counterparty_identifier_snapshot ni
-- counterparty_name_snapshot en payments, por lo que la tabla
-- de Cobros mostraba "—" en las columnas RUT/NIC y Nombre.
--
-- Patrón correcto (igual que trade_docs):
--   payments.counterparty_id                  → FK a counterparties
--   payments.counterparty_identifier_snapshot → snapshot del RUT
--   payments.counterparty_name_snapshot       → snapshot del nombre
--
-- Este archivo:
--   1. Elimina la columna description (nunca usada; se usa notes).
--   2. Backfill de los snapshots para pagos con NULL, resolviendo
--      payment_allocations → trade_docs → (counterparties si falta).
--   3. Actualiza process_other_doc_import_batch para que los pagos
--      futuros incluyan counterparty_id y snapshots desde ya.
-- =========================================================

-- ── 1. Eliminar columna description (duplica notes, nunca consultada) ─────────
ALTER TABLE public.payments DROP COLUMN IF EXISTS description;

-- ── 2a. Backfill desde trade_docs via payment_allocations ─────────────────────
--    Para cada pago sin snapshots, tomar el counterparty del primer
--    trade_doc asociado (ordenado por created_at de la allocation).
UPDATE public.payments p
SET
  counterparty_identifier_snapshot = COALESCE(
    p.counterparty_identifier_snapshot,
    td_data.counterparty_identifier_snapshot
  ),
  counterparty_name_snapshot = COALESCE(
    p.counterparty_name_snapshot,
    td_data.counterparty_name_snapshot
  ),
  counterparty_id = COALESCE(
    p.counterparty_id,
    td_data.counterparty_id
  )
FROM (
  SELECT DISTINCT ON (pa.payment_id)
    pa.payment_id,
    td.counterparty_identifier_snapshot,
    td.counterparty_name_snapshot,
    td.counterparty_id
  FROM public.payment_allocations pa
  JOIN public.trade_docs td ON td.id = pa.trade_doc_id
  WHERE td.counterparty_identifier_snapshot IS NOT NULL
     OR td.counterparty_id IS NOT NULL
  ORDER BY pa.payment_id, pa.created_at ASC
) td_data
WHERE p.id = td_data.payment_id
  AND (
    p.counterparty_identifier_snapshot IS NULL
    OR p.counterparty_name_snapshot IS NULL
    OR p.counterparty_id IS NULL
  );

-- ── 2b. Si counterparty_id está pero los snapshots siguen NULL ────────────────
--    (puede pasar si trade_docs tampoco tenía snapshot pero sí tenía FK)
UPDATE public.payments p
SET
  counterparty_identifier_snapshot = COALESCE(p.counterparty_identifier_snapshot, c.identifier),
  counterparty_name_snapshot       = COALESCE(p.counterparty_name_snapshot,       c.name)
...
-- ── 3. Actualizar RPC: agrega counterparty_id y snapshots en INSERT payments ──
