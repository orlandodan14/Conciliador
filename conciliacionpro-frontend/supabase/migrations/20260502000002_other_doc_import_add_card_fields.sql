-- =========================================================
-- 20260502000002_other_doc_import_add_card_fields.sql
--
-- Actualiza process_other_doc_import_batch para incluir
-- los campos de tarjeta: card_kind, card_last4, auth_code.
-- =========================================================
...
DECLARE
  ...
  v_card_kind          text;   -- ← nuevo
  v_card_last4         text;   -- ← nuevo
  v_auth_code          text;   -- ← nuevo
    ...
    BEGIN
      ...
      IF v_doc_type = 'DEVOLUCION' AND v_origin_number IS NOT NULL THEN
        ...
        WHERE td.company_id = p_company_id
          ...
          AND (
            v_origin_doc_type IS NULL
            OR upper(td.doc_type) = upper(v_origin_doc_type)
          )
          AND (
            v_origin_fdc IS NULL
            OR td.fiscal_doc_code::text = v_origin_fdc
          )
