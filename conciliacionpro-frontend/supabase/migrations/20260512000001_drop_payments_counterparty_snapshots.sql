-- =========================================================
-- 20260512000001_drop_payments_counterparty_snapshots.sql
--
-- Optimización: payments ya tiene counterparty_id (FK).
-- No es necesario duplicar identifier y name como snapshot.
-- La UI resolverá RUT/nombre haciendo JOIN con counterparties
-- usando la FK, igual que en un diseño relacional limpio.
--
--  1. Elimina counterparty_identifier_snapshot de payments.
--  2. Elimina counterparty_name_snapshot de payments.
--  3. Actualiza process_other_doc_import_batch para que el
--     INSERT de payments ya no incluya esos campos.
-- =========================================================

-- ── 1-2. Eliminar columnas redundantes ────────────────────────────────────────
ALTER TABLE public.payments DROP COLUMN IF EXISTS counterparty_identifier_snapshot;
ALTER TABLE public.payments DROP COLUMN IF EXISTS counterparty_name_snapshot;
