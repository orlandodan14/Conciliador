-- =============================================================
-- 20260514000002_bulk_imports_add_je_counterparty.sql
--
-- Agrega counterparty_id al INSERT de journal_entries en las
-- tres RPCs de carga masiva:
--   · process_cobro_import_batch
--   · process_other_doc_import_batch
--   · process_trade_doc_import_job
-- =============================================================

-- ─────────────────────────────────────────────────────────────
-- 1. process_cobro_import_batch
-- ─────────────────────────────────────────────────────────────
  ...
  FOR v_doc IN SELECT jsonb_array_elements(p_docs) LOOP
    ...
    BEGIN
      ...
      INSERT INTO public.journal_entry_lines (
        ...
        v_line_desc, abs(v_total_amount), 0,
      ...
      INSERT INTO public.journal_entry_lines (
        ...
        v_line_desc, 0, abs(v_total_amount),
        v_branch_haber_id, v_bl_haber_id
      );

      INSERT INTO public.payments (
        company_id, payment_type, status, method,
        card_kind, card_last4, auth_code,
        payment_date, reference, notes,
        total_amount, currency_code,
        counterparty_id, journal_entry_id, extra
      ) VALUES (
        p_company_id, v_payment_type, 'BORRADOR',
      ...
      IF v_alloc_doc_number IS NOT NULL THEN
        ...
        IF v_alloc_doc_id IS NOT NULL THEN
          INSERT INTO public.payment_allocations (
            ...
            p_company_id, v_payment_id, v_alloc_doc_id,
...
-- ─────────────────────────────────────────────────────────────
-- 2. process_other_doc_import_batch
-- ─────────────────────────────────────────────────────────────
  ...
  FOR v_doc IN SELECT jsonb_array_elements(p_docs) LOOP
    ...
    BEGIN
      ...
      IF v_branch_code IS NOT NULL THEN
        ...
        WHERE company_id = p_company_id AND upper(trim(code)) = upper(v_branch_code)
        LIMIT 1;
      END IF;

      -- ── 6. Resolver doc de origen (solo DEVOLUCION) ───────────
      ...
      IF v_pay_amount > 0 AND v_pay_method IS NOT NULL THEN
        INSERT INTO public.payments (
          ...
          payment_date, method, total_amount,
          reference, card_kind, card_last4, auth_code,
          currency_code, notes, journal_entry_id, extra,
          created_by, counterparty_id
        ) VALUES (
          p_company_id,
          CASE WHEN v_doc_type = 'DEVOLUCION' THEN 'PAGO' ELSE 'COBRO' END,
          'VIGENTE',
          coalesce(v_pay_date, v_issue_date),
          v_pay_method, v_pay_amount,
          v_pay_reference, v_card_kind, v_card_last4, v_auth_code,
          v_currency, null, v_je_id,
          jsonb_build_object('source', 'trade_docs_non_fiscal', 'trade_doc_id', v_doc_id),
          null, v_cp_id
        )
...
-- ─────────────────────────────────────────────────────────────
-- 3. process_trade_doc_import_job
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- 12) Crear journal entry draft con counterparty_id
      INSERT INTO public.journal_entries (
        company_id, entry_date, description, reference,
