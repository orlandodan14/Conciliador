-- =========================================================
-- 20260507000003_fix_payment_type_by_doc_type.sql
--
-- PROBLEMA DETECTADO:
-- El RPC process_other_doc_import_batch inserta payments para
-- TANTO OTRO_INGRESO (cobros de ingresos) COMO DEVOLUCION
-- (pagos a clientes por devolución) sin diferenciar.
-- Todos quedaron con payment_type = 'COBRO' (DEFAULT), siendo
-- que los vinculados a DEVOLUCION deben ser payment_type = 'PAGO'.
--
-- REGLA DE NEGOCIO:
--   payment → allocation → trade_doc.doc_type = 'DEVOLUCION'
--     → payment_type = 'PAGO'    (egreso: le pagamos al cliente)
--   payment → allocation → trade_doc.doc_type = 'OTRO_INGRESO' / 'INVOICE' / etc.
--     → payment_type = 'COBRO'   (ingreso: el cliente nos paga)
--
-- Este archivo:
--   1. Corrige registros EXISTENTES (backfill).
--   2. Actualiza el RPC para que futuros imports sean correctos.
-- =========================================================

-- ── 1. Backfill: COBRO → PAGO ─────────────────────────────────────────────────
--    Payments cuyas allocations apuntan a documentos DEVOLUCION.

UPDATE public.payments p
SET payment_type = 'PAGO'
WHERE p.payment_type = 'COBRO'
  AND EXISTS (
    SELECT 1
    ...
    WHERE pa.payment_id = p.id
      ...
      AND td.doc_type   = 'DEVOLUCION'
  );

-- ── 2. Backfill: AJUSTE_COBRO → AJUSTE_PAGO ──────────────────────────────────
--    Ajustes vinculados a documentos DEVOLUCION.

UPDATE public.payments p
SET payment_type = 'AJUSTE_PAGO'
WHERE p.payment_type = 'AJUSTE_COBRO'
  AND EXISTS (
    SELECT 1
    FROM public.payment_allocations pa
    JOIN public.trade_docs td ON td.id = pa.trade_doc_id
...
-- ── 3. Actualizar RPC para que futuras importaciones sean correctas ───────────
--    Único cambio respecto a la versión anterior:
--    • Se agrega payment_type al INSERT (PAGO si DEVOLUCION, COBRO si no).
--    • Se agrega status = 'VIGENTE' explícitamente (ya era el default,
...
BEGIN
  ...
  FOR v_doc IN SELECT jsonb_array_elements(p_docs) LOOP
    ...
    BEGIN
      ...
      --   payment_type: PAGO si es devolución, COBRO si es otro ingreso.
      --   status: VIGENTE (pago ya aplicado al registrar el documento).
      IF v_pay_amount > 0 AND v_pay_method IS NOT NULL THEN
        INSERT INTO public.payments (
          company_id,
          payment_type,                           -- ← COBRO o PAGO según doc_type
          status,                                  -- ← VIGENTE (pago ya aplicado)
          payment_date,
          method,
          total_amount,
          reference,
