-- =========================================================
-- 20260513000002_timeline_payment_status.sql
--
-- Corrige get_trade_doc_timeline para que los eventos de
-- tipo PAYMENT devuelvan el estado real del cobro
-- (BORRADOR / VIGENTE / CANCELADO) en lugar del valor
-- hardcodeado 'APPLIED'.
--
-- También ajusta affects_label según el estado del cobro:
--   BORRADOR  → "Pago pendiente de registro"
--   CANCELADO → "Pago cancelado"
--   VIGENTE   → "Pago aplicado al documento relacionado"
-- =========================================================
...
payment_events as (
  select
    ...
    -- Label según estado real del cobro
    case p.status
      when 'BORRADOR'  then 'Pago pendiente de registro'
      when 'CANCELADO' then 'Pago cancelado'
      else                  'Pago aplicado al documento relacionado'
    end as affects_label,
    ...
    p.status::text as item_status   -- ← estado real: BORRADOR / VIGENTE / CANCELADO
...
GRANT EXECUTE ON FUNCTION public.get_trade_doc_timeline(uuid, uuid)
