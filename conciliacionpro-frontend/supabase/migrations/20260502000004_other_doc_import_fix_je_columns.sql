-- =========================================================
-- 20260502000004_other_doc_import_fix_je_columns.sql
--
-- Corrige las columnas de journal_entries y journal_entry_lines
-- en process_other_doc_import_batch:
--
--  journal_entries:
--    ANTES (incorrecto): source, source_id, accounting_period_id
--    AHORA (correcto)  : accounting_period_id, currency_code,
--                        created_by, extra (JSONB con source info)
--
--  journal_entry_lines:
--    ANTES (incorrecto): description
--    AHORA (correcto)  : line_description, account_code_snapshot,
...
DECLARE
  ...
  -- nombres de cuenta para snapshots
  ...
  v_line_desc          text;
BEGIN
  -- Período contable activo más reciente (puede ser null — campo nullable)
