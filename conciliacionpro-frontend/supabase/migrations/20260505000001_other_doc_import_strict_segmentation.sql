-- =========================================================
-- 20260505000001_other_doc_import_strict_segmentation.sql
--
-- Corrige process_other_doc_import_batch para que falle
-- EXPLÍCITAMENTE cuando un código de sucursal o línea de
-- negocio se provee pero no se encuentra en la BD, en lugar
-- de crear silenciosamente la línea contable sin segmentación.
--
-- También añade validación: si la cuenta exige SUC/CU
-- (account_imputation_policies.require_suc / require_cu)
-- y no se proporcionó el código correspondiente, se lanza
-- excepción antes de insertar nada.
-- =========================================================
...
DECLARE
  ...
  -- Políticas de imputación para las cuentas del asiento
...
BEGIN
  ...
  FOR v_doc IN SELECT jsonb_array_elements(p_docs) LOOP
    ...
    BEGIN
      ...
      -- ── 7b. Leer políticas de imputación de las cuentas ────────────────────
      ...
      WHERE  company_id = p_company_id
        AND  account_node_id = v_acc_debe_id
      LIMIT 1;
      ...
      FROM   public.account_imputation_policies
      WHERE  company_id = p_company_id
        AND  account_node_id = v_acc_haber_id
      LIMIT 1;
      v_req_suc_haber := coalesce(v_req_suc_haber, false);
      v_req_cu_haber  := coalesce(v_req_cu_haber,  false);

      -- Validar: si la cuenta exige SUC/CU, el código debe estar provisto
      IF v_req_suc_debe AND v_br_debe_code IS NULL THEN
        RAISE EXCEPTION 'Cuenta DEBE "%" exige Sucursal (SUC) pero branch_code_debe está vacío (fila %)', v_acc_debe_code, v_row_no;
      END IF;
      IF v_req_cu_debe AND v_bl_debe_code IS NULL THEN
        RAISE EXCEPTION 'Cuenta DEBE "%" exige Centro de Utilidad (CU) pero business_line_code_debe está vacío (fila %)', v_acc_debe_code, v_row_no;
      END IF;
      IF v_req_suc_haber AND v_br_haber_code IS NULL THEN
        RAISE EXCEPTION 'Cuenta HABER "%" exige Sucursal (SUC) pero branch_code_haber está vacío (fila %)', v_acc_haber_code, v_row_no;
      END IF;
      IF v_req_cu_haber AND v_bl_haber_code IS NULL THEN
        RAISE EXCEPTION 'Cuenta HABER "%" exige Centro de Utilidad (CU) pero business_line_code_haber está vacío (fila %)', v_acc_haber_code, v_row_no;
      END IF;

      -- ── 8. Resolver sucursales de línea ─────────────────────────────────────
      -- STRICT: si se provee código, debe existir; si no existe → error
      ...
      IF v_br_debe_code IS NOT NULL THEN
        ...
        IF v_branch_debe_id IS NULL THEN
          RAISE EXCEPTION 'branch_code_debe "%" no encontrado en catálogo de sucursales (fila %)', v_br_debe_code, v_row_no;
        END IF;
      END IF;
      IF v_br_haber_code IS NOT NULL THEN
        ...
        IF v_branch_haber_id IS NULL THEN
          RAISE EXCEPTION 'branch_code_haber "%" no encontrado en catálogo de sucursales (fila %)', v_br_haber_code, v_row_no;
        END IF;
      END IF;

      -- ── 9. Resolver business lines ──────────────────────────────────────────
      -- STRICT: mismo criterio
      v_bl_debe_id  := NULL;
      v_bl_haber_id := NULL;
      IF v_bl_debe_code IS NOT NULL THEN
        ...
        IF v_bl_debe_id IS NULL THEN
          RAISE EXCEPTION 'business_line_code_debe "%" no encontrado en catálogo (fila %)', v_bl_debe_code, v_row_no;
        END IF;
      END IF;
      IF v_bl_haber_code IS NOT NULL THEN
        ...
        IF v_bl_haber_id IS NULL THEN
          RAISE EXCEPTION 'business_line_code_haber "%" no encontrado en catálogo (fila %)', v_bl_haber_code, v_row_no;
        END IF;
      END IF;
